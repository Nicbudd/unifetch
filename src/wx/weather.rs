...
async fn current_conditions_handler(config: &Config) -> Result<String, String> {
    ...
    for x in config.weather.selected.sources.iter() {
        ...
        let db: BTreeMap<DateTime<Utc>, WxEntry> = 
        ...
        let local_time: DateTime<Local> = latest.0.clone().into();
        let prelude = format!("{}: ⌛{}", station.name, local_time.format("%I:%M %p"));
        let line = station_line(&prelude, latest.1, 
            &config.weather.selected.parameters, indoor, &db)?;
        s.push_str(&line)
    }
    ...
    // let apt_line = station_line(&apt_prelude, latest_apt.1, &config.weather.selected.parameters, true, &apt_db)?;
    // let psm_line = station_line(&psm_prelude, latest_psm.1, &config.weather.selected.parameters, false, &psm_db)?;
    // let unh_line = station_line(&unh_prelude, latest_unh.1, &config.weather.selected.parameters, false, &unh_db)?;
